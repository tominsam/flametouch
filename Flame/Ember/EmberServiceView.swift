...
struct EmberServiceView: View {
    var viewModel: EmberServiceViewModel
        ...
        List {
            Section("Core") {
                ...
                if let domain = viewModel.service?.domain {
                    EmberValueCell(title: "Domain", subtitle: domain)
                }

                ForEach(viewModel.service?.addressCluster.sorted ?? [], id: \.self) { address in
                    EmberValueCell(title: "Address", subtitle: address)
                }

                EmberValueCell(title: "Port", subtitle: String(viewModel.service?.port ?? 0))
            }
            ...
            if !sortedData.isEmpty {
                Section("Data") {
                    ForEach(sortedData, id: \.key) { data in
                        EmberValueCell(title: data.key, subtitle: data.value, url: realUrl(from: data.value))
                    }
                }
            }
        }
